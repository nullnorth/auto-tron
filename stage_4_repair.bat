...
REM title Tron v%TRON_VERSION% [stage_4_repair] [SFC Scan]
REM call functions\log.bat "%CUR_DATE% %TIME%    Launch job 'System File Checker'..."
REM if /i %DRY_RUN%==no (
	...
	REM if %WIN_VER_NUM% geq 6.0 (
		REM %SystemRoot%\System32\sfc.exe /scannow
		REM %SystemRoot%\System32\findstr.exe /c:"[SR]" %SystemRoot%\logs\cbs\cbs.log>> "%LOGPATH%\%LOGFILE%" 2>NUL
	REM )
REM )
...
REM title Tron v%TRON_VERSION% [stage_4_repair] [chkdsk]
REM call functions\log.bat "%CUR_DATE% %TIME%    Launch job 'chkdsk'..."
REM call functions\log.bat "%CUR_DATE% %TIME%    Checking %SystemDrive% for errors..."
REM :: Run a read-only scan and look for errors. Schedule a scan at next reboot if errors found
REM if /i %DRY_RUN%==no %SystemRoot%\System32\chkdsk.exe %SystemDrive%
REM if /i not %ERRORLEVEL%==0 (
	REM call functions\log.bat "%CUR_DATE% %TIME% !  Errors found on %SystemDrive%. Scheduling full chkdsk at next reboot."
	REM if /i %DRY_RUN%==no fsutil dirty set %SystemDrive%
REM ) else (
	REM call functions\log.bat "%CUR_DATE% %TIME%    No errors found on %SystemDrive%. Skipping full chkdsk at next reboot."
REM )
REM call functions\log.bat "%CUR_DATE% %TIME%    Done."
...
REM title Tron v%TRON_VERSION% [stage_4_repair] [repair file extensions]
REM call functions\log.bat "%CUR_DATE% %TIME%    Launch job 'Repair file extensions'..."
REM if /i %DRY_RUN%==no (
	REM setlocal
	REM call stage_4_repair\repair_file_extensions\repair_file_extensions.bat
	REM endlocal
REM )
REM call functions\log.bat "%CUR_DATE% %TIME%    Done."
