...
REM title Tron v%TRON_VERSION% [stage_1_tempclean] [CCleaner]
REM call functions\log.bat "%CUR_DATE% %TIME%    Launch job 'CCleaner'..."
REM if /i %DRY_RUN%==no (
	REM if /i %VERBOSE%==yes call functions\log.bat "%CUR_DATE% %TIME% !  VERBOSE (-v) output requested but not supported by CCleaner. Sorry."
...
:: JOB: BleachBit
REM title Tron v%TRON_VERSION% [stage_1_tempclean] [BleachBit]
REM call functions\log.bat "%CUR_DATE% %TIME%    Launch job 'BleachBit'..."
REM if /i %DRY_RUN%==no (

	REM if %VERBOSE%==yes (
		REM :: OK yes, this is wonky. If verbose is requested we first dump all files to the screen, THEN dump them to the log, THEN do the actual clean
		REM :: Thanks Windows Batch for not having a TEE or equivalent
		REM stage_1_tempclean\bleachbit\bleachbit_console.exe --preset --preview
		REM stage_1_tempclean\bleachbit\bleachbit_console.exe --preset --preview>> "%LOGPATH%\%LOGFILE%" 2>NUL
	REM )

	REM stage_1_tempclean\bleachbit\bleachbit_console.exe --preset --clean >> "%LOGPATH%\%LOGFILE%" 2>NUL
	REM ping 127.0.0.1 -n 12 >NUL
REM )
REM call functions\log.bat "%CUR_DATE% %TIME%    Done."
